...
class Wallet_System_For_Woocommerce_Admin {
			...
			if ( isset( $user_id, $nonce ) && wp_verify_nonce( $nonce, 'view_report_' . $user_id ) ) {
				...
				wp_localize_script(
					...
					array(
						...
						'user_data_cashback' => $user_data['cashback_amount'],
						'user_data_withdrawal' => $user_data['withdrawal_amount'],
	...
	public function wps_wsfw_get_user_report( $user_id, $start_date, $end_date ) {
		...
		// wallet withdrawal.
			...
			$wps_wallet_withdrawal_amount = 0;
	...
	public function wsfw_admin_general_settings_page( $wsfw_settings_general ) {
		...
		$wsfw_settings_general = array(
			...
			array(
				'title'       => __( 'Enable to Auto Refund to Wallet for Wallet Payments.', 'wallet-system-for-woocommerce' ),
				'type'        => 'radio-switch',
				'description' => __( 'Enable if you want to autorefund order paid by wallet gateway on refund order status.', 'wallet-system-for-woocommerce' ),
				'name'        => 'wsfw_wallet_payment_refund_order_payment',
				'id'          => 'wsfw_wallet_payment_refund_order_payment',
				'value'       => '',
				'class'       => 'wsfw-radio-switch-class',
				'options'     => array(
					'yes' => __( 'YES', 'wallet-system-for-woocommerce' ),
	...
	public function wsfw_admin_wallet_action_settings_comment_array( $wsfw_settings_template ) {
		$wsfw_settings_template = array(
			...
			array(
				...
				'step'        => '0.01',
				'value'       => ! empty( get_option( 'wps_wsfw_wallet_action_comment_description' ) ) ? get_option( 'wps_wsfw_wallet_action_comment_description' ) : 'Enter Comment Description',
	...
	public function wsfw_admis_save_tab_settings_for_kyc() {
		...
		if ( isset( $_POST['wsfw_button_wallet_kyc_tab_option'] ) ) {
			...
			if ( wp_verify_nonce( $nonce ) ) {
				...
				$wps_wsfw_gen_flag     = false;
				$wsfw_genaral_settings = apply_filters( 'wsfw_wallet_kyc_notification_settings', array() );
				$wsfw_button_index     = array_search( 'submit', array_column( $wsfw_genaral_settings, 'type' ) );
				if ( isset( $wsfw_button_index ) && ( null == $wsfw_button_index || '' == $wsfw_button_index ) ) {
					$wsfw_button_index = array_search( 'button', array_column( $wsfw_genaral_settings, 'type' ) );
				}
	...
	public function wsfw_admin_save_data( $wsfw_genaral_settings, $wps_wsfw_gen_flag ) {
		global $wsfw_wps_wsfw_obj;
		if ( isset( $_POST['wsfw_button_wallet_action'] ) || isset( $_POST['wsfw_button_cashback'] ) || isset( $_POST['wsfw_button_wallet_withdrawal_wbnpl_tab_option'] ) || isset( $_POST['wsfw_button_wallet_kyc_tab_option'] ) ) {
			...
			$nonce_kyc = ( isset( $_POST['updatenoncewallet_kyc'] ) ) ? sanitize_text_field( wp_unslash( $_POST['updatenoncewallet_kyc'] ) ) : '';
			if ( wp_verify_nonce( $nonce_action ) || wp_verify_nonce( $nonce_cashback ) || wp_verify_nonce( $nonce_bpnl ) || wp_verify_nonce( $nonce_kyc ) ) {
				$wsfw_button_index     = array_search( 'submit', array_column( $wsfw_genaral_settings, 'type' ) );
				if ( isset( $wsfw_button_index ) && ( null == $wsfw_button_index || '' == $wsfw_button_index ) ) {
					$wsfw_button_index = array_search( 'button', array_column( $wsfw_genaral_settings, 'type' ) );
				}
	...
	public function wsfw_wallet_sms_notification_settings_tab( $wsfw_settings_template ) {
		...
		$wsfw_settings_template   = apply_filters( 'wsfw_wallet_action_comment_extra_settings_array', $wsfw_settings_template );
		$wsfw_settings_template[] = array(
			'title'       => __( 'Enable Wallet SMS Notification', 'wallet-system-for-woocommerce' ),
			'type'        => 'radio-switch',
			'description' => '',
			'name'        => 'wps_wsfwp_wallet_sms_notification_enable',
			'id'          => 'wps_wsfwp_wallet_sms_notification_enable',
			'value'       => '',
			'class'       => 'wsfw-radio-switch-class wps_pro_settings',
			'options'     => array(
				'yes' => __( 'YES', 'wallet-system-for-woocommerce' ),
		...
		$wsfw_settings_template[] = array(
			...
			'value'       => '',
			'placeholder' => __( 'Please enter account SID', 'wallet-system-for-woocommerce' ),
			'class'       => 'wws-text-class wps_pro_settings',

		);
		$wsfw_settings_template[] = array(
			'title'       => __( 'Enter Twilio Account Auth Token', 'wallet-system-for-woocommerce' ),
			'type'        => 'text',
			'description' => __( 'Please enter Twilio Account Auth Token', 'wallet-system-for-woocommerce' ),
			'name'        => 'wps_wsfwp_wallet_sms_notification_auth_token',
			'id'          => 'wps_wsfwp_wallet_sms_notification_auth_token',
			'value'       => '',
			'placeholder' => __( 'Please enter account auth token', 'wallet-system-for-woocommerce' ),
			'class'       => 'wws-text-class wps_pro_settings',
		...
		$wsfw_settings_template[] = array(
			...
			'value'       => '',
			'placeholder' => __( 'Please enter account phone number', 'wallet-system-for-woocommerce' ),
			'class'       => 'wws-text-class wps_pro_settings',
	...
	/**
	 * This is used to enable wallet BNPL.
	 *
	 * @param array $wsfw_settings_template setting template.
	 * @return array
	 */
	public function wsfw_wallet_bnpl_notification_settings_tab( $wsfw_settings_template ) {
		...
		if ( $is_pro_plugin ) {
			$setting_description = __( 'Enable to allow customers to use wallet with Buy Now, Pay Later.', 'wallet-system-for-woocommerce' );
		} else {

			$setting_description = __( 'Enable to allow admin to use wallet with Buy Now, Pay Later.', 'wallet-system-for-woocommerce' );
		}
		...
		$wsfw_settings_template[] = array(
			'title'       => __( 'Enter name to add on checkout page for interest of WBNPL', 'wallet-system-for-woocommerce' ),
			'type'        => 'text',
		...
		$wsfw_settings_template[] = array(
			'title'       => __( 'WBNPL interest Type for negative balance', 'wallet-system-for-woocommerce' ),
			'type'        => 'select',
			'description' => __( 'Select type of interest on WBNPL during Balance recharge.', 'wallet-system-for-woocommerce' ),
		...
		$wsfw_settings_template[] = array(
			'title'       => __( 'Enter WBNPL interest Amount for negative balance', 'wallet-system-for-woocommerce' ),
			'type'        => 'number',
			'description' => __( 'Charge interest on WBNPL during Balance recharge.', 'wallet-system-for-woocommerce' ),
	...
	/**
	 * This is used to enable wallet Kyc.
	 *
	 * @param array $wsfw_settings_template setting template.
	 * @return array
	 */
	public function wsfw_wallet_kyc_notification_settings_tab( $wsfw_settings_template ) {
		...
		$wsfw_settings_template[] = array(
			'title'       => __( 'Enable Wallet KYC', 'wallet-system-for-woocommerce' ),
			'type'        => 'radio-switch',
			'description' => __( 'Enable to use wallet with KYC.', 'wallet-system-for-woocommerce' ),
			'name'        => 'wsfw_enable_wallet_kyc',
			'id'          => 'wsfw_enable_wallet_kyc',
			'value'       => get_option( 'wsfw_enable_wallet_kyc', 'no' ),
			'class'       => 'wsfw-radio-switch-class',
			'options'     => array(
				'yes' => __( 'YES', 'wallet-system-for-woocommerce' ),
				'no'  => __( 'NO', 'wallet-system-for-woocommerce' ),
