jQuery(document).ready(function() {
	...
	// Wallet collapsible panel toggle (Option C)
	jQuery(document).on('click', '.wps-wallet-panel-toggle', function() {
		var toggle = jQuery(this);
		var content = toggle.siblings('.wps-wallet-panel-content');
		var arrow = toggle.find('.wps-wallet-panel-arrow');
		var isCollapsed = toggle.attr('data-collapsed') === 'true';
		...
		if (isCollapsed) {
			...
			content.slideUp(300);
			arrow.text('▼');
			toggle.attr('data-collapsed', 'true');
		}
	});
});
