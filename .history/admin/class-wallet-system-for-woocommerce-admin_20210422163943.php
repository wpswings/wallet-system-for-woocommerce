	...
	*/
	...
	public function wsfw_save_user_wallet_field( $user_id ) {
		if ( current_user_can( 'edit_user', $user_id) ) {
			...
			$mail_message = sprintf( "Hello %s,<br/>", $name );
			$mail_message1     = __( 'Merchant has credited your wallet by '. wc_price( $wallet_amount ), 'wallet-system-for-woocommerce' );
			...
			if ( $update ) {
				...
				if ( 'credit' === $action ) {
					...
					$mail_message     = __( 'Merchant has deducted '. wc_price( $wallet_amount ). ' from your wallet by '. wc_price( $wallet_amount ), 'wallet-system-for-woocommerce' );
				}
	...
	public function custom_code_in_head() {
		...
		.woocommerce_page_wallet_shop_order #ui-datepicker-div {background: #fff;padding: 15px;font-size:15px;border-radius: 5px;}
