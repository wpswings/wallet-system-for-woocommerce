		...
		if($wpdb->get_var("SHOW TABLES LIKE '$older_table'") == $older_table) {
			...
			if ( ! empty( $user_transactions ) && is_array( $user_transactions ) ) {
				foreach ( $user_transactions as $user_transaction ) {
					...
					VALUES ( '" . $user_transaction->transaction_id . "' , '" . $user_transaction->user_id . "', '" . $transactiondata['amount'] . "', '" . $transactiondata['transaction_type'] . "', '". $transactiondata['payment_method'] . "', '". $transactiondata['order_id'] . "', '". $transactiondata['note'] . "', NOW() )";
