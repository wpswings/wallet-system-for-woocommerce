...
class Wallet_System_For_Woocommerce_Admin {
	...
	*/
		...
		if ( current_user_can( 'edit_user', $user_id) ) {
			...
			if ( $update ) {
				...
				if ( 'credit' === $action ) {
					...
					$mail_message     = __( 'Merchant has credited your wallet by '. wc_price( $wallet_amount ), 'wallet-system-for-woocommerce' );
				} elseif ( 'debit' === $action ) {
					...
					$mail_message     = __( 'Merchant has deducted '. wc_price( $wallet_amount ). ' from your wallet.', 'wallet-system-for-woocommerce' );
				}
	...
	/**
	 * Saving the plugin seeting to new option name
	 *
	...
	public function wsfw_upgrade_completed() {
		...
		// update custom wallet payment gateway
		$wallet_payment_gateway = get_option( 'woocommerce_mwb_wcb_wallet_payment_gateway_settings', array() );
		update_option( 'woocommerce_mwb_wsfw_wallet_payment_gateway_settings', $wallet_payment_gateway );
		...
		if($wpdb->get_var("SHOW TABLES LIKE '$older_table'") == $older_table) {
			...
			if ( ! empty( $user_transactions ) && is_array( $user_transactions ) ) {
				foreach ( $user_transactions as $user_transaction ) {
					$user = get_user_by( 'id', $user_transaction->user_id );

					$insert = "INSERT INTO  " . $table_name . "
					( Id, user_id, amount, transaction_type, payment_method, transaction_id, note, date ) 
					VALUES ( '" . $user_transaction->transaction_id . "' , '" . $user_transaction->transaction_id . "''" . $transactiondata['amount'] . "', '" . $transactiondata['transaction_type'] . "', '". $transactiondata['payment_method'] . "', '". $transactiondata['order_id'] . "', '". $transactiondata['note'] . "', NOW() )";
