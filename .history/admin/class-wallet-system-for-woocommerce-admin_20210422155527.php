...
class Wallet_System_For_Woocommerce_Admin {
	...
	*/
	...
	public function wsfw_save_user_wallet_field( $user_id ) {
		$number = 9;
$str = "Beijing";
$txt = esc_html__( sprintf("There are %u million bicycles in %s.",$number,$str), 'wallet-system-for-woocommerce' );
echo $txt;
		if ( current_user_can( 'edit_user', $user_id) ) {
			...
			if ( $update ) {
				...
				if ( 'credit' === $action ) {
					...
					$mail_message     = esc_html__( 'Your wallet is credited by ', 'wallet-system-for-woocommerce' );
				} elseif ( 'debit' === $action ) {
