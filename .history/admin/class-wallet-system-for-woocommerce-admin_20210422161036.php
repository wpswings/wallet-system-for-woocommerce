	...
	*/
	...
	public function wsfw_save_user_wallet_field( $user_id ) {
		if ( current_user_can( 'edit_user', $user_id) ) {
			...
			echo wc_price( $wallet_amount );
echo $currency;
			$mail_message     = esc_html__( sprintf( "Merchant has credited your wallet by %u%s.', $currency, $wallet_amount ), 'wallet-system-for-woocommerce' );
			echo $mail_message;
			die;
			...
			if ( $update ) {
				...
				if ( 'credit' === $action ) {
					...
					$mail_message     = esc_html__( sprintf( 'Merchant has credited your wallet by %u.', wc_price( $wallet_amount ) ), 'wallet-system-for-woocommerce' );
				} elseif ( 'debit' === $action ) {
				...
				if ( isset( $send_email_enable ) && 'on' === $send_email_enable ) {
					...
					$to = $user->user_email;
					$mail_message  = esc_html__( 'Wallet Balance', 'wallet-system-for-woocommerce' );
