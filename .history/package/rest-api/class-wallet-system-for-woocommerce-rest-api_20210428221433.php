...
class Wallet_System_For_Woocommerce_Rest_Api {
	 ...
	 * @since    1.0.0
	 */
	public function mwb_wsfw_get_permission_check( WP_REST_Request $request ) {

		$rest_api_keys = get_option( 'mwb_wsfw_wallet_rest_api_keys', '' );
    	if ( ! empty( $rest_api_keys )  && is_array( $rest_api_keys ) ){ 
			$key     = $request->get_header('consumer_key');
			$secret  = $request->get_header('consumer_secret');
			return new WP_Error( 'rest_forbidden', esc_html__( $key, 'wallet-system-for-woocommerce' ), array( 'status' => 401 ) );
