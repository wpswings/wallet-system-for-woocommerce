...
class Wallet_System_For_Woocommerce_Rest_Api {
	...
	public function mwb_wsfw_add_endpoint() {
		...
		// for getting particular user wallet details or transactions
		register_rest_route(
			$namespace,
			$base_url . '(?P<id>\d+)',
			array(
	...
	public function mwb_wsfw_default_permission_check( $request ) {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			return new WP_Error( 'rest_forbidden', esc_html__( 'Sorry, you cannot list resources..', 'my-text-domain' ), array( 'status' => 401 ) );
		}
