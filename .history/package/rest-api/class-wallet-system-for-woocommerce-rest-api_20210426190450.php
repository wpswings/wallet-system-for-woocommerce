...
class Wallet_System_For_Woocommerce_Rest_Api {
	...
	public function mwb_wsfw_add_endpoint() {
		...
		register_rest_route(
			...
			array(
				...
				'callback' => array( $this, 'mwb_wsfw_user_wallet_balance' ),
				'permission_callback' => array( $this, 'mwb_wsfw_create_permission_check' ),
			)
		);
	...
	public function mwb_wsfw_create_permission_check( $request ) {

		// Add rest api validation for each request.
		// if ( ! current_user_can( 'manage_woocommerce' )  ) {
		// 	return new WP_Error( 'rest_forbidden', esc_html__( 'Sorry, you are not allowed to edit wallet.', 'wallet-system-for-woocommerce' ), array( 'status' => 401 ) );
		// }
		return true;
	}
