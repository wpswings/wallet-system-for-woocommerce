			...
			if ( ! empty( $user_id ) ) {
				...
				if ( $user ) {
					...
					if ( 'credit' == $wallet_action ) { 
						...
						$mail_message     = __( 'Your wallet has been debited by '. wc_price( $transfer_amount ) . ' through wallet transfer toMerchant has deducted '. wc_price( $updated_amount ). ' from your wallet.', 'wallet-system-for-woocommerce' );
