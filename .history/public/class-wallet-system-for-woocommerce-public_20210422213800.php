...
class Wallet_System_For_Woocommerce_Public {
	...
	public function mwb_wcb_credit_wallet_callback() {
		$get_nonce = isset( $_REQUEST['mwb-wcb-credit-wallet-nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['mwb-wcb-credit-wallet-nonce'] ) ) : '';

		if ( wp_verify_nonce( $get_nonce, 'mwb-wcb-credit-wallet-nonce' ) ) {
			if ( isset( $_POST['mwb_wallet_topup_add'] ) && ! empty( $_POST['mwb_wallet_topup_add'] ) ) {
				if ( isset( $_POST['wallet_topup_add'] ) && ! empty( $_POST['wallet_topup_add'] ) ) {
					add_filter( 'woocommerce_add_cart_item_data', array( $this, 'add_wallet_topup_product_in_cart' ), 10, 4 );
					$product_id = $this->mwb_wcb_topup_pro_id;
					WC()->cart->empty_cart();
