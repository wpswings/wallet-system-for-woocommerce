    ...
    if ( $update ) {
        ...
        if ( $returnid ) {
            ...
            if ( $update_user ) {
                ...
                $mail_text2.= __( 'Wallet credited by '. wc_price( $transfer_amount ) . ' through wallet transfer by .' . $name2, 'wallet-system-for-woocommerce' );
                $to2 = $user2->user_email;
                $headers2 = 'From: '. $from . "\r\n" .
                    'Reply-To: ' . $to2 . "\r\n";

                //Here put your Validation and send mail
                $sent2 = wp_mail( $to2, $subject, $mail_text1, $headers2 );
                if( $sent2 ) {
                    echo 'message send';
                }//message sent!
                else  {
