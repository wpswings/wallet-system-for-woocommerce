    ...
    if ( $update ) {
        ...
        if ( $returnid ) {
            ...
            if ( isset( $send_email_enable ) && 'on' === $send_email_enable ) {
                ...
                $name2 = $user2->first_name . ' ' . $user1->last_name; 

                $mail_text1 = sprintf( "Hello %s,<br/>", $name1 );
                $mail_text1 .= __( 'Wallt credited by '. wc_price( $transfer_amount ) . ' through wallet transfer.', 'wallet-system-for-woocommerce' );
                $to1 = $user1->user_email;
                $from = get_option( 'admin_email' );
                $subject1 = "Wallet updating notification";
                $headers1 = 'From: '. $from . "\r\n" .
                    'Reply-To: ' . $to . "\r\n";

                //Here put your Validation and send mail
                $sent1 = wp_mail( $to1, $subject1, $mail_text1, $headers1 );
                if( $sent ) {
                    echo 'message send';
                }//message sent!
                else  {
