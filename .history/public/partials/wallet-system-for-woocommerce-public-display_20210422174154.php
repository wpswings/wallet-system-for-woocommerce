    ...
    if ( $update ) {
        ...
        if ( $returnid ) {
            ...
            if ( isset( $send_email_enable ) && 'on' === $send_email_enable ) {
                ...
                $mail_text1 .= __( 'Wallt credited by '. wc_price( $transfer_amount ) . ' through wallet transfer by .' . $name2, 'wallet-system-for-woocommerce' );
                ...
                $sent1 = wp_mail( $to1, $subject, $mail_text1, $headers1 );
                if( $sent1 ) {
                    echo 'message send';
                }//message sent!
                else  {
            ...
            if ( $update_user ) {
                ...
                $mail_text2 = sprintf( "Hello %s,<br/>", $name1 );
                $mail_text12.= __( 'Wallt credited by '. wc_price( $transfer_amount ) . ' through wallet transfer by .' . $name2, 'wallet-system-for-woocommerce' );
                $to1 = $user1->user_email;
                $from = get_option( 'admin_email' );
                $subject = "Wallet updating notification";
                $headers1 = 'From: '. $from . "\r\n" .
